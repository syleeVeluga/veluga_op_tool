...
### 비IT 사용자 사용성 점검 (2026-02-15)

- **판정**: 현재 MVP 기준으로 비IT 사용자도 사용 가능
- **근거**
  - 고객 검색 자동완성으로 ID 직접 입력 부담 완화
  - `billing_logs` 선택 시 기본 필터(`deletedState=active`)와 기본 컬럼 자동 적용
  - 결제 금액 안내 문구로 정산 해석 오해 방지(요금제 기준 금액, 실결제/환불 미지원)
- **운영 권장 사용 순서(초심자용)**
  1. `고객 검색(자동완성)`에서 사용자 선택
  2. 기간(Start/End)만 먼저 지정
  3. `로그 조회` 실행 후 필요 시 필터 추가
  4. 결과 확인 후 CSV/JSON 다운로드
- **남은 개선 포인트(후속)**
  - 영문 라벨(`Data Type`, `Customer ID`, `Include Total`) 한글화
  - 용어 단순화(내부 키명 대신 업무 용어 우선 노출)
